...
const habitablePlanets = planets.filter((planet) => {
  let acceptableTemperature = planet.temperature >= 253 && planet.temperature <= 323;
  let acceptableDistance = planet.distance >= 0.75 && planet.distance <= 1.5;
  return acceptableTemperature && acceptableDistance;
});
