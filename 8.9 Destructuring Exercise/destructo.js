...
// TODO: Destructo has found a map with coordinates marked on it.
// Use object destructuring to extract the `x` and `y` coordinates from the given `coordinates` object.
...
console.log(`First location: ${first}, second location: ${second}`);
...
console.log(`Upper: ${upper}, middle: ${middle}, lower: ${lower}`);
...
// TODO: Crossing the River of Reflections requires skipping certain stones.
// Use array destructuring to extract only the first and the sixth stones.
// Print the extracted stones.

const [firstStone, , , , , sixthStone] = stones;
console.log(`First stone: ${firstStone}, sixth stone: ${sixthStone}`);
...
console.log(
  `Visible shadow: ${visibleShadow}, hidden shadows: ${hiddenShadows}`
);
...
revealPath({ direction: "South", distance: 200 });
...
// Write a function `mixPotion` that uses defaults "Water" and "Fireflower" for `ingredient1` and `ingredient2` if they are not specified and print those mixings.
// The function takes an object with these optional properties.

function mixPotion({ ingredient1 = "Water", ingredient2 = "Fireflower" } = {}) {
  console.log(`Mixing ${ingredient1} and ${ingredient2}`);
}

mixPotion({ ingredient1: "Newt's Tail" });
...
function castSpell([firstIngredient, secondIngredient]) {
  console.log(
    `Casting a spell with ${firstIngredient} and ${secondIngredient}`
  );
}

castSpell(["Bat's Wing", "Unicorn Blood", "Deer Horn"]);
...
// Use nested destructuring to extract `The Final Key`.
...
// Use array destructuring to swap the values of `stoneA` and `stoneB`.
